...
<td><input type="password" class="form-control" id="pass" placeholder="Enter ypur password"><br>
</div>
...
<button type="submit" class="btn btn-success" onclick="update(datas[i]['id'])">UPDATE</button>
</form>
</div>
</div>
<script>
...
for(i in datas){
document.getElementById("name2").value=datas[i]['name'];
document.getElementById("email2").value=datas[i]['email'];
document.getElementById("pass2").value=datas[i]['pass'];
}
})
}
...
{
tblRow += "<tr id='row'>"
tblRow += "<td id='id1'>"+data[i]['id']+"</td><td id='name1'>"+data[i]['name']+"</td><td id='email1'>"+data[i]['email']+"</td><td id='pass1'>"+data[i]['pass']+"</td>";
tblRow += "<td><button class='btn btn-success mr-2' id='edit' data-toggle='modal' data-target='#mymodal2' onclick='select(\""+data[i]['id']+"\")'>Edit</button></td>"
tblRow += "<td><button class='btn btn-danger ml-2' onclick='deletevalue(\""+data[i]['id']+"\")'>delete</button></td>"
tblRow += "</tr>";
}
}
tblRow +="</tbody></table>";
...
swal("Data Not Inserted!","Error", "Error");
}
})
}
...
if(data=="success"){
swal("Data updated successfully!","success", "success");
getdata();
}
else{
